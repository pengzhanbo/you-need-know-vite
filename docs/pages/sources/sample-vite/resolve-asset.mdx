...
使用 [开发服务](/sources/sample-vite/http-server) 可以正确加载 `index.css`和`main.js` ，但是 `@module/js-cookie` 时，却遇到了问题， `@module/js-cookie` 不是一个标准的资源路径，当前的 **开发服务** 不能正确读取到该资源，
对浏览器来说，也无法判断该资源的类型。
...
此时，浏览器可以正确读取并识别资源类型。接下来，则是实现 资源更新重载。
